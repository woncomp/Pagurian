...
def save_frames(frames, output):
    ...
    for index, (elapsed, frame) in enumerate(frames):
        ...
        if elapsed < 3.1 and frame.height > 20 and frame.width >= 220:
            row = [frame.getpixel((x, 20)) for x in range(8, 220)]
            teal = [x + 8 for x, (r, g, b) in enumerate(row)
...
def main():
    ...
    output = args.output or exe.parent / ("capture-taskbar" if args.taskbar else "capture-isolated")
